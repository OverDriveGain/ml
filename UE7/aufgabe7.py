...
def bootstrap(data, labels, cycles=1000):
    X_train, X_test, y_train, y_test = train_test_split(data, labels, test_size=0.2, random_state=30, stratify=labels)

    dataSize = len(data)
    errors = []
    for i in range(cycles):
        indices = np.random.choice(range(len(y_train)), dataSize)
        sampleSet = X_train[indices]
        sampleLabel = y_train[indices]
        Baum = ID3Tree(sampleSet, sampleLabel)

        richtig = 0
        falsch = 0
        for x in range(len(X_test)):

            if Baum.predict(X_test[x]) == y_test[x]:
                richtig += 1
            else:
                falsch += 1

        errors.append(falsch / (richtig + falsch))

    print("Mittlerer Fehler: " + str(sum(errors)/len(errors) * 100 )  +" %")
    print("Max Fehler: " + str(max(errors) * 100 )+" %")
    print("Min Fehler: " + str(min(errors) * 100 )+" %")
